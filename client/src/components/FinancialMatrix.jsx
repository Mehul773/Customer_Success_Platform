...
function FinancialMatrix({ project, setFetch, myUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .post(`/matrix/create-financialMatrix/${project._id}`, formData)
        .then((res) => {
          if (res.status === 200) {
            setFetch((prev) => !prev);
    ...
    if (a) {
      try {
        const response = await axios.delete(
          `/matrix/delete-financialMatrix/${project._id}/${financialMatrix_id}`
        );
      ...
      {myUser && (
        <>
          {(myUser?.role === "Admin" || myUser?.role === "PM") && (
            <Button onClick={openModal} className="m-2" color="positive">
              + Add Financial Escalation Matrixxxx
            </Button>
          )}
